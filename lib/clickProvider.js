...
var selectorFactory = {
  ...
  lFunction : {
    ...
    targetPath : function () {
      var i18nPath = path.join(util.getTiProjectRootPath(),"i18n")
    ...
    didGenerateCallbacks ({callbacks,text}){
      ...
      var i18nStringPath = path.join(util.getTiProjectRootPath(),"i18n",defaultLang,"strings.xml");
        ...
        callbacks.push({
          ...
          callback (){
            ...
            if(!util.isExistAsFile(i18nStringPath)){ //if not, create i18n/strings.xml
              mkdirp.sync(path.join(util.getTiProjectRootPath(),"i18n",defaultLang));
