...
function findDefinition(path,regExp){
  ...
  targetBuffer.scan(regExp,function(item){
    callbacks.push({
      title : item.lineText,
      callback (){
        atom.workspace.open(path,{initialLine: item.range.start.row, initialColumn: item.range.start.column, searchAllPanes:true});
      }
    });
  });
...
var xmlClickProvider = {
  ...
  getSuggestionForWord(textEditor: TextEditor, text: string, range: Range): HyperclickSuggestion {
    ...
    callbacks = findDefinition(related.getTargetPath(matchedExpItem.targetType), matchedExpItem.getTargetRegExp(text));
